...
class CrossAttention_Diffusion_LM(nn.Module):
    def __init__(self, config, out_channels, vocab_size=None):
        ...
        config.hidden_size = int(config.intermediate_size / 4)
        ...
        if config.encoder.initialize_from_pretrained:
            if dist.get_rank() == 0:
                logger.info(f"Load pretrained bert encoder with {encoder_cfg.num_hidden_layers} layers.")
            ...
            if dist.get_rank() == 0:
                logger.info("Load random bert encoder with {encoder_cfg.num_hidden_layers} layers.")
        ...
        if config.pred_len:
            ...
            self.word_embedding = nn.Embedding(config.vocab_size, config.in_channels, config.pad_value)
        else:
            self.word_embedding = nn.Embedding(config.vocab_size, config.in_channels, None)

        self.register_buffer("position_ids", torch.arange(config.max_pos_len).expand((1, -1)))
        self.position_embeddings = nn.Embedding(config.max_pos_len, self.hidden_size)
        
        self.embed_scale = math.sqrt(config.in_channels) if self.config.scale_embedding else 1.0
        self.LayerNorm = nn.LayerNorm(self.hidden_size, eps=encoder_cfg.layer_norm_eps)
        ...
        if self.config.predict_x_start:
            self.lm_transform = nn.Sequential(
                ...
                nn.GELU(),
                nn.LayerNorm(config.in_channels, eps=encoder_cfg.layer_norm_eps)
            )
        self.lm_head = nn.Linear(config.in_channels, config.vocab_size)
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock(
                    ...
                    num_attention_heads=config.denoiser.num_attention_heads,
                    attention_head_dim=self.hidden_size // config.denoiser.num_attention_heads,
                    dropout=config.dropout,
                    attention_dropout=config.denoiser.att_dropout,
                    cross_attention_dim=self.hidden_size,
                    activation_fn="geglu",
                    attention_bias=False,
                )
                for d in range(config.denoiser.layers)
            ]
        )
